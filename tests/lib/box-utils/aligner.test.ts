...
describe('createAligner', () => {
  it('centers rect over box center', () => {
    ...
    expect(aligner.align({ width: 30, height: 10 })).toEqual({ left: 25, top: 7, width: 30, height: 10 });
  });

  it('top-left over top-left gives origin', () => {
    const aligner = createAligner(box, 'tl', 'tl');
    expect(aligner.align({ width: 30, height: 10 })).toEqual({ left: 0, top: 0, width: 30, height: 10 });
  });

  it('bottom-right over bottom-right aligns correctly', () => {
    const aligner = createAligner(box, 'br', 'br');
    expect(aligner.align({ width: 30, height: 10 })).toEqual({ left: 50, top: 14, width: 30, height: 10 });
  });

  it('rect bottom-middle over explicit point', () => {
    const aligner = createAligner(box, 'bm', [40, 20]);
    expect(aligner.align({ width: 30, height: 10 })).toEqual({ left: 25, top: 10, width: 30, height: 10 });
  });

  it('rect top-left over box center', () => {
    const aligner = createAligner(box, 'tl', 'c');
    expect(aligner.align({ width: 30, height: 10 })).toEqual({ left: 40, top: 12, width: 30, height: 10 });
  });

  it('preserves rect dimensions in result', () => {
    const aligner = createAligner(box, 'c', 'c');
    const result = aligner.align({ width: 44, height: 6 });
  ...
  it('allows overflow (left/top may be negative)', () => {
    ...
    expect(aligner.align({ width: 30, height: 10 })).toEqual({ left: -15, top: -5, width: 30, height: 10 });
  });
  ...
  describe('anchor aliases', () => {
    it('lt resolves same as tl', () => {
      ...
      expect(a.align({ width: 30, height: 10 })).toEqual(b.align({ width: 30, height: 10 }));
    });
  ...
  describe('anchor rounding suffixes', () => {
    it('tm.+ uses ceil on odd rect width', () => {
      ...
      const resultCeil = a.align({ width: 7, height: 6 });
      const resultFloor = b.align({ width: 7, height: 6 });
    ...
    it('.u is alias for .+', () => {
      ...
      expect(a.align({ width: 7, height: 6 })).toEqual(b.align({ width: 7, height: 6 }));
    });
  ...
  describe('axis expressions', () => {
    it('r and b resolve to box width and height', () => {
      ...
      expect(aligner.align({ width: 10, height: 5 })).toEqual({ left: 80, top: 24, width: 10, height: 5 });
    });

    it('r - 2 subtracts from box width', () => {
      const aligner = createAligner(box, 'tl', ['r - 2', 10]);
      expect(aligner.align({ width: 10, height: 5 })).toEqual({ left: 78, top: 10, width: 10, height: 5 });
    });
    ...
    it('c is axis-aware: x→bw/2, y→bh/2 (floor)', () => {
      ...
      expect(aligner.align({ width: 1, height: 1 })).toEqual({ left: 40, top: 12, width: 1, height: 1 });
    });
    ...
    it('(c).+ ceil on odd box', () => {
      ...
      expect(aligner.align({ width: 1, height: 1 })).toEqual({ left: 41, top: 13, width: 1, height: 1 });
    });
    ...
    it('(r / 3).- floor on 80-wide box', () => {
      ...
      // floor(80/3)=26
      expect(aligner.align({ width: 1, height: 1 })).toEqual({ left: 26, top: 5, width: 1, height: 1 });
    });

    it('(r / 3).+ ceil on 80-wide box', () => {
      const aligner = createAligner(box, 'tl', ['(r / 3).+', 5]);
      // ceil(80/3)=27
      expect(aligner.align({ width: 1, height: 1 })).toEqual({ left: 27, top: 5, width: 1, height: 1 });
    });

    it('t and l resolve to 0', () => {
      const aligner = createAligner(box, 'tl', ['t', 'l']);
      expect(aligner.align({ width: 5, height: 5 })).toEqual({ left: 0, top: 0, width: 5, height: 5 });
    });

    it('t + 5 gives 5', () => {
      const aligner = createAligner(box, 'tl', ['t + 5', 'l + 3']);
      expect(aligner.align({ width: 1, height: 1 })).toEqual({ left: 5, top: 3, width: 1, height: 1 });
    });
  });
});
