...
export class Utex {
  readATC(data, offset, img, w, h)
  {
    ...
    for(var y=0; y<h; y+=4)
      for(var x=0; x<w; x+=4)
      {
        this.readATCcolor(data, offset, sqr);
  ...
  readATA(data, offset, img, w, h)
  {
    ...
    for(var y=0; y<h; y+=4)
      for(var x=0; x<w; x+=4) {
        this.readATCcolor(data, offset+8, sqr);
        /*
  ...
  readBC3(data, offset, img, w, h)
  {
    ...
    for(var y=0; y<h; y+=4)
      for(var x=0; x<w; x+=4)
      {
        ...
        var al = this.inter8(data[offset], data[offset+1]);	pos.boff+=16;
        for(var i=0; i<64; i+=4) {
  ...
  read4x4(a, w, h, sx,sy, b)	// read from large
  {
    for(var y=0; y<4; y++) {
  ...
  write4x4(a, w, h, sx,sy, b)	// write to large
  {
    for(var y=0; y<4; y++) {
      var si = ((sy+y)*w+sx)<<2, ti = y<<4;
  ...
  readBC7(data, offset, img, w, h)
  {
    ...
    var subs = [ null, null, this._subs2, this._subs3 ];
    var ancs = [ null, null, this._anch2, this._anch3 ];
  ...
  {
    var out = 0, ok=k;
    while(k!=0) {  out = (out) | (this.readBit(data, pos)<<(ok-k));  k--;  }
    return out;
  }
  ...
  colorDist(r,g,b, r0,g0,b0) {  return (r-r0)*(r-r0)+(g-g0)*(g-g0)+(b-b0)*(b-b0);  }
