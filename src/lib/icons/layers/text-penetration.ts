...
const defaultAligner = createAligner(PogsConstants, 'rt', ['r', 2]);

export function textPenetration(app: WGData, aligner: Aligner = defaultAligner): LayerFactory {
  return async (_box, _prev, vehicle) => {
    const profiles = await app.getStatsForBestConfig(vehicle) as Record<string, VehicleProfile>;
    const text = profiles[vehicle.tank_id]?.ammo?.[0].penetration?.[1];
    if (text === undefined) {
      return null;
    }
